...
const HeaderTemp4 = forwardRef((props, ref) => {
    ...
    <>
      <div
        ...
        className={classNames(
          " group h-auto bg-temp4bg sm:py-4 sm:px-8",
          fontFam?.name
        )}
      >
        <nav
              ...
              >
                <ul className="items-center justify-center space-y-8 md:flex md:space-x-16 md:space-y-0">
                  {dataObj.data.navigation.map((item, index) => (
                    <li
                      key={item.name}
                      className="text-white hover:text-sky-600 font-poppins font-thin text-md sm:text-xl"
                    >
