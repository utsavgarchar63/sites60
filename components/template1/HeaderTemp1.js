...
const HeaderTemp1 = forwardRef((props, ref) => {
  ...
  return (
    <>
      <div
        ref={ref}
        className={classNames(" group h-auto bg-white py-3 px-3 sm:px-8 ", fontFam?.name)}
      >
        <nav
            ...
            <div>
              <div className="flex items-center justify-between py-3 md:py-5 md:block">
                <a href="javascript:void(0)">
                  <h2 className="text-3xl font-bold text-temp1text">
                    {dataObj?.data?.companyName}
                  </h2>
                </a>
                <div className="md:hidden">
                  <button
                    className="p-2 text-temp1text rounded-md outline-none focus:border-gray-400 focus:border"
              ...
              >
                <ul className="items-center justify-center space-y-8 md:flex md:space-x-16 md:space-y-0">
                  {dataObj.data.navigation.map((item, index) => (
                    <li
                      key={item.name}
                      className={`text-${index === 0 ? "temp1bg" : "custom-gray text-opacity-80"} hover:text-sky-600 text-md sm:text-xl`}
                    >
